...
$refusalId=intval($_POST['refusal_id']??0);if($refusalId<=0){http_response_code(400);header('Content-Type: application/json');echo json_encode(['error'=>'Missing refusal_id']);exit;}
...
db()->prepare("INSERT INTO refusal_files (refusal_id,uploaded_by_staff_id,original_name,stored_name,mime_type,size_bytes) VALUES (?,?,?,?,?,?)")
  ->execute([$refusalId,intval($user['id']),$orig,$stored,$mime,intval($f['size'])]);
$fileId=intval(db()->lastInsertId());
audit_log(intval($user['id']),'UPLOAD_FILE','refusal',$refusalId,null,null,['file_id'=>$fileId,'original_name'=>$orig,'mime'=>$mime,'size'=>$f['size']]);
