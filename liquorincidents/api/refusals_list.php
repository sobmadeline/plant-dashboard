...
$sql="SELECT r.*, s.name AS created_by_name FROM refusals r LEFT JOIN staff_users s ON s.id=r.created_by_staff_id";
$where=[];$params=[];
if($month){$where[]="DATE_FORMAT(r.refusal_date,'%Y-%m') = ?";$params[]=$month;}
if($q){$where[]="(r.refusal_no LIKE ? OR r.location LIKE ? OR r.reason LIKE ?)";$params[]='%'.$q.'%';$params[]='%'.$q.'%';$params[]='%'.$q.'%';}
